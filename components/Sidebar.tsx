    ...
    <div className="w-2/12 bg-[#212121]  overflow-auto pb-8 sidebar">
      ...
      <ul className="flex flex-col border-b-2 border-gray-700">
        {secondaryLinks.map((link) => {
          return (
            <li key={link.name} className={`pl-6  py-3 hover:bg-zinc-600 `}>
              <a href="#" className="flex items-center gap-5 ">
                {link.icon}
                <span className="text-sm tracking-wider">{link.name}</span>
              </a>
      ...
      <br />
      <div className="pt-3 px-4 text-sm text-zinc-400">
        Cloned for Educational purposes
      </div>
    </div>
  );
};
